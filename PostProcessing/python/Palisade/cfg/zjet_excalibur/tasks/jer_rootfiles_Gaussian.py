...
def cli(argument_parser):
    ...
    argument_parser.add_argument('--output-format',
                                 ...
                                 default='JER_truncated_Gaussian_Z{channel}_{sample}_{jec}_{corr_level}/{plot_label}.pdf')
...
def run(args):
    ...
    if args.output_dir is None:
        args.output_dir = "JER_truncated_Gaussian_{}".format(datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%f"))
    ...
    for channel in args.channels:
        ...
        _cfg = get_config(
            ...
            output_format=(args.output_format if not args.root else
                           'JER_truncated_Gaussian_Z{channel}_{sample}_{jec}_{corr_level}.root'),
            root=args.root
        )
        if args.root:

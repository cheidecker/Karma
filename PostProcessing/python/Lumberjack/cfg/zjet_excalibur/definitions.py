...
for _prefix in ("z", "jet1", "jet2", "jet3"):
    QUANTITIES['global'].update({
        '{}eta'.format(_prefix): Quantity(
            ...
            binning=[-5.191, -3.839, -3.489, -3.139, -2.964, -2.853, -2.650, -2.500, -2.322, -2.172, -1.930, -1.653,
                     -1.479, -1.305, -1.044, -0.783, -0.522, -0.261, 0.000, 0.261, 0.522, 0.783, 1.044, 1.305, 1.479,
                     1.653, 1.930, 2.172, 2.322, 2.500, 2.650, 2.853, 2.964, 3.139, 3.489, 3.839, 5.191],
        ),
        'abs{}eta'.format(_prefix): Quantity(
            name='abs{}eta'.format(_prefix),
            expression='abs({}eta)'.format(_prefix),
            binning=[0.000, 0.261, 0.522, 0.783, 1.044, 1.305, 1.479, 1.653, 1.930, 2.172, 2.322, 2.500, 2.650, 2.853,
...
_ALPHA_BIN_EDGES_JER = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
...
SPLITTINGS['eta'] = {"absEta_{:04d}_{:04d}".format(int(_ETA_BIN_EDGES_BARREL[0]*1e3),
                                                   int(_ETA_BIN_EDGES_BARREL[1]*1e3)):
                     dict(absjet1eta=_ETA_BIN_EDGES_BARREL)}
SPLITTINGS['eta'].update(SPLITTINGS['eta_narrow'])
SPLITTINGS['eta'].update(SPLITTINGS['eta_wide'])

SPLITTINGS['eta_wide_barrel'] = {"absEta_{:04d}_{:04d}".format(int(_ETA_BIN_EDGES_BARREL[0]*1e3),
                                                               int(_ETA_BIN_EDGES_BARREL[1]*1e3)):
                                 dict(absjet1eta=_ETA_BIN_EDGES_BARREL)}
